...
export function InquiryForm({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <section className={embedded ? '' : 'rounded-2xl border border-[var(--border)] bg-[var(--card)] p-6 md:p-8'}>
      {!embedded ? (
        <>
          <h1 className="font-serif text-2xl font-normal tracking-tight">Have a question?</h1>
          <p className="mt-3 text-sm font-light text-[var(--muted)]">
            Send us your question and we&apos;ll reply with practical next steps.
          </p>
        </>
      ) : null}
      ...
      <form className={embedded ? 'space-y-6' : 'mt-8 space-y-6'} onSubmit={handleSubmit} noValidate>
        <div className="space-y-2">
          <label htmlFor="io-email" className="text-sm font-medium">
            Email
          </label>
          <input
            id="io-email"
            ...
            aria-describedby={errors.email ? 'io-email-error' : undefined}
            className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm outline-none ring-[var(--accent)] transition focus:ring-2 disabled:cursor-not-allowed disabled:opacity-70"
            required
          />
          {errors.email ? (
            <p id="io-email-error" className="text-sm text-red-600 dark:text-red-300">
              {errors.email}
            </p>
          ) : null}
        </div>
        ...
        <div className="space-y-2">
          <label htmlFor="io-name" className="text-sm font-medium">
          ...
          <input
            ...
            aria-describedby={errors.name ? 'io-name-error' : undefined}
            className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm outline-none ring-[var(--accent)] transition focus:ring-2 disabled:cursor-not-allowed disabled:opacity-70"
            maxLength={120}
          />
          {errors.name ? (
            <p id="io-name-error" className="text-sm text-red-600 dark:text-red-300">
              {errors.name}
            </p>
          ) : null}
        </div>
        ...
        <div className="space-y-2">
          <label htmlFor="io-question" className="text-sm font-medium">
            Question
          </label>
          <textarea
            id="io-question"
            ...
            aria-describedby={errors.question ? 'io-question-error' : 'io-question-help'}
            rows={8}
            className="w-full rounded-lg border border-[var(--border)] bg-[var(--background)] px-3 py-2 text-sm outline-none ring-[var(--accent)] transition focus:ring-2 disabled:cursor-not-allowed disabled:opacity-70"
            required
          />
          <p id="io-question-help" className="text-xs text-[var(--muted)]">
            {questionLength}/5000 characters (minimum 10)
          </p>
          {errors.question ? (
            <p id="io-question-error" className="text-sm text-red-600 dark:text-red-300">
              {errors.question}
            </p>
          ) : null}
        </div>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="inline-flex items-center rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-70"
        >
