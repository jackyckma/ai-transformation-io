...
export function ArticleListItem({ page, category }: ArticleListItemProps) {
  const label = category ?? PILLAR_LABEL[page.pillar];
  ...
  return (
    <article className="border-b border-[var(--border)] py-8 first:pt-0 last:border-b-0">
      <Link href={page.pathname} className="group block">
        <p className="text-xs font-light tracking-wide text-[var(--muted)]">{label}</p>
