...
export function NewsletterSubscribe({ list, label }: NewsletterSubscribeProps) {
  ...
  return (
    <form className="flex flex-col gap-2" onSubmit={handleSubmit} noValidate>
      <label htmlFor={fieldId} className="text-sm font-normal text-[var(--secondary)]">
        {label}
      </label>
      <div className="flex max-w-sm items-center gap-2">
        ...
        <button
          type="submit"
          disabled={pending}
          className="inline-flex shrink-0 items-center rounded-lg border border-[var(--border)] px-3 py-2 text-sm font-normal text-[var(--foreground)] transition hover:border-[var(--accent)] hover:text-[var(--accent)] disabled:cursor-not-allowed disabled:opacity-70"
        >
