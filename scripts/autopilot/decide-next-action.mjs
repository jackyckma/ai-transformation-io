...
if (laneArg === "maker") {
  ...
  // The stale-lock check above (and, for Checker specifically, the one below)
  // is what catches the case where this keeps happening tick after tick —
  // real open PRs whose lock age crosses staleLockHours get escalated
  // regardless of why REVIEW never got to them, without needing a separate
  // failure counter for this one specific command.
